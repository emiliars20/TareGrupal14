document.addEventListener('DOMContentLoaded', function () {
  ...
  form.addEventListener('submit', function (event) {
      ...
      if (areFieldsEmpty()) {
          alert('Por favor complete todos los campos obligatorios: nombre, apellido, email, contraseña y términos.');
          event.preventDefault(); // Evita el envío del formulario si están vacíos
          return;
      }
  ...
  // Función para verificar si todos los campos obligatorios están vacíos
  function areFieldsEmpty() {
      // Comprueba que los campos nombre, apellido y email no estén vacíos
      return nombre.value.trim() === '' || apellido.value.trim() === '' || email.value.trim() === '';
  }
});
